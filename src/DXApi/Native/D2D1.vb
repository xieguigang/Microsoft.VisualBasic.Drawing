...
'''
''' NOTE: every com interface here is declared as a flat interface, the
''' methods of the native base interface are repeated in the derived
''' interface declaration. the clr resolves the interface pointer of a
''' derived interface through QueryInterface, and a com interface that
''' inherits another com interface can not be resolved correctly in the
''' interop stub, so the vtable of each interface is written out in full
''' here to keep the slot layout explicit and safe.
''' </remarks>
Friend Module D2D1
...
<InterfaceType(ComInterfaceType.InterfaceIsIUnknown)>
Friend Interface ID2D1GradientStopCollection
    ' slot 3, ID2D1Resource
    <PreserveSig> Sub GetFactory(<Out> ByRef factory As IntPtr)
End Interface
...
<InterfaceType(ComInterfaceType.InterfaceIsIUnknown)>
Friend Interface ID2D1StrokeStyle
    ' slot 3, ID2D1Resource
    <PreserveSig> Sub GetFactory(<Out> ByRef factory As IntPtr)
End Interface
...
<InterfaceType(ComInterfaceType.InterfaceIsIUnknown)>
Friend Interface ID2D1Geometry
...
<InterfaceType(ComInterfaceType.InterfaceIsIUnknown)>
Friend Interface ID2D1RectangleGeometry

    ' slot 3, ID2D1Resource
    <PreserveSig> Sub GetFactory(<Out> ByRef factory As IntPtr)
    ' slot 4 .. slot 16, ID2D1Geometry
    <PreserveSig> Function GetBounds(worldTransform As IntPtr, <Out> ByRef bounds As D2D1_RECT_F) As Integer
    <PreserveSig> Function GetWidenedBounds(strokeWidth As Single, strokeStyle As ID2D1StrokeStyle, worldTransform As IntPtr, flatteningTolerance As Single, <Out> ByRef bounds As D2D1_RECT_F) As Integer
    <PreserveSig> Function StrokeContainsPoint(point As D2D1_POINT_2F, strokeWidth As Single, strokeStyle As ID2D1StrokeStyle, worldTransform As IntPtr, flatteningTolerance As Single, <Out> ByRef contains As Integer) As Integer
    <PreserveSig> Function FillContainsPoint(point As D2D1_POINT_2F, worldTransform As IntPtr, flatteningTolerance As Single, <Out> ByRef contains As Integer) As Integer
...
<InterfaceType(ComInterfaceType.InterfaceIsIUnknown)>
Friend Interface ID2D1EllipseGeometry
...
<InterfaceType(ComInterfaceType.InterfaceIsIUnknown)>
Friend Interface ID2D1PathGeometry
...
<InterfaceType(ComInterfaceType.InterfaceIsIUnknown)>
Friend Interface ID2D1GeometrySink

    ' slot 3 .. slot 9, ID2D1SimplifiedGeometrySink
    <PreserveSig> Sub SetFillMode(fillMode As D2D1_FILL_MODE)
    <PreserveSig> Sub SetSegmentFlags(vertexFlags As Integer)
    <PreserveSig> Sub BeginFigure(startPoint As D2D1_POINT_2F, figureBegin As D2D1_FIGURE_BEGIN)
    <PreserveSig> Sub AddLines(<MarshalAs(UnmanagedType.LPArray)> points As D2D1_POINT_2F(), count As UInteger)
